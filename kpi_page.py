    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        try:
            ...
            if role == Qt.ForegroundRole and self._mask is not None:
                try:
                    if row < len(self._mask) and col < len(self._mask.columns):
                        if bool(self._mask.iat[row, col]):
            ...
            print(f"[ERROR] DataFrameModel.data() failed: {e}")
            return None
...
class RangeFilterProxy(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, source_row, source_parent):
        try:
            ...
            print(f"[ERROR] RangeFilterProxy.filterAcceptsRow() failed: {e}")
            return True  # エラー時はフィルタを通す

    def lessThan(self, left: QModelIndex, right: QModelIndex) -> bool:
        """
        ソート時の比較を実装。数値、日時、文字列を適切に比較する。
        """
        try:
            model = self.sourceModel()
            if not model:
            ...
            # EditRoleで元の値を取得（数値や日時がそのまま取得できる）
            left_val = model.data(left, Qt.EditRole)
            right_val = model.data(right, Qt.EditRole)

            # None や NaN の処理
            if left_val is None or (isinstance(left_val, float) and math.isnan(left_val)):
                return False  # None/NaN は常に後ろに
            if right_val is None or (isinstance(right_val, float) and math.isnan(right_val)):
                return True  # None/NaN は常に後ろに
            ...
            # 日時の比較
            if isinstance(left_val, (pd.Timestamp, datetime.datetime)) and isinstance(right_val, (pd.Timestamp, datetime.datetime)):
                return bool(left_val < right_val)

            # Timedelta の比較
            if isinstance(left_val, pd.Timedelta) and isinstance(right_val, pd.Timedelta):
                return bool(left_val < right_val)

            # 文字列比較（フォールバック）
            left_str = str(left_val) if left_val is not None else ""
            right_str = str(right_val) if right_val is not None else ""
            return bool(left_str < right_str)
        except Exception as e:
            print(f"[ERROR] RangeFilterProxy.lessThan() failed: {e}")
            return False  # エラー時は順序を変えない
...
class KPIJsonEditorPage(QWidget):
    ...
    def __init__(self, kpi_page: "KPIPage"):
        ...
        try:
            ...
            try:
                if kpi_page.df_all is not None and not kpi_page.df_all.empty:
            ...
            print(f"[ERROR] KPIJsonEditorPage.__init__() failed: {e}")
            import traceback
            traceback.print_exc()
            raise
    ...
    def _on_save_clicked(self):
        try:
            ...
            try:
                ...
                import traceback
                traceback.print_exc()
                QMessageBox.warning(self, "警告", f"設定は保存されましたが、表示の更新に失敗しました:\n{e}")
            ...
            print(f"[ERROR] _on_save_clicked() failed: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.warning(self, "エラー", f"保存処理中にエラーが発生しました:\n{e}")
...
class KPIPage(QWidget):
    ...
    def __init__(self, query: str, stacked_widget, user_ids):
        ...
        try:
            ...
            try:
                ...
                print(f"[ERROR] Failed to parse user_ids: {e}")
                self._user_ids = []
            ...
            print(f"[ERROR] KPIPage.__init__() failed: {e}")
            import traceback
            traceback.print_exc()
            # 最低限の初期化
            self.df_all = pd.DataFrame()
    ...
    def _load_data_and_prepare_kpi(self):
        ...
        try:
            ...
            self.df_all = pd.DataFrame()
            self._interval_config = {}
            QMessageBox.critical(self, "データ読み込みエラー", f"データの読み込みに失敗しました:\n{e}")
    ...
    def _ensure_interval_columns(self):
        ...
        """
        try:
            if self.df_all.empty or self.df_all is None:
                return
            ...
            for mode, entries in cfg.items():
                ...
                for ent in entries:
                    ...
                    try:
                        ...
                        # None や NaN を適切に処理する
                        # s や e_series に None が含まれている可能性があるため、NaN に変換
                        ...
                        print(f"[ERROR] Failed to process interval {ent}: {e}")
                        continue
        except Exception as e:
            print(f"[ERROR] _ensure_interval_columns() failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    def go_back(self):
        try:
            ...
            print(f"[ERROR] go_back() failed: {e}")
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            if "__row_id" in self.df_all.columns:
                try:
                    ...
                    print(f"[ERROR] Failed to delete rows by __row_id: {e}")
                    # フォールバック: インデックスで削除
                    self.df_all.drop(self.df_all.index[source_rows], inplace=True, errors='ignore')
            else:
                self.df_all.drop(self.df_all.index[source_rows], inplace=True, errors='ignore')
            ...
            print(f"[ERROR] delete_selected_rows() failed: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.warning(self, "削除エラー", f"行の削除に失敗しました:\n{e}")
    ...
    def export_current_view_to_csv(self):
        try:
            ...
            print(f"[ERROR] export_current_view_to_csv() failed: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.warning(self, "Export Failed", f"エクスポートに失敗しました:\n{e}")
    ...
    def _copy_selected_rows_to_clipboard(self):
        try:
            ...
            rows = sorted({i.row() for i in sel.selectedIndexes() if i.isValid()})
            if not rows:
                rows = sorted(i.row() for i in sel.selectedRows() if i.isValid())
            ...
            print(f"[ERROR] _copy_selected_rows_to_clipboard() failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _reload_kpi(self):
        try:
            ...
            print(f"[ERROR] _reload_kpi() failed: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.warning(self, "リロードエラー", f"データのリロードに失敗しました:\n{e}")
    ...
    def _open_kpi_json_editor(self):
        ...
        try:
            ...
            print(f"[ERROR] _open_kpi_json_editor() failed: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.warning(self, "エラー", f"KPI設定エディタを開けませんでした:\n{e}")
